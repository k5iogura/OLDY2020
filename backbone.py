...
class MBConvBlock(nn.Module):
    ...
    References:
        [1] https://arxiv.org/abs/1905.02244 (MobileNet v3)
    """
...
class EfficientNet(nn.Module):
    """EfficientNet model.
       Most easily loaded with the .from_name method.
    Args:
    ...
    def forward(self, inputs):
        ...
        x = self._dropout(x)
        x = self._fc(x)
        assert False,"Assume : dont use backbone.EfficientNet.forward()"
        return x
    ...
    def _check_model_name_is_valid(cls, model_name):
        ...
        """
        valid_models = ['efficientnet-lite' + str(i) for i in range(9)]
